...
public interface ReviewDao {
    ...
    //@Insert("insert into \"HONG\".\"TEST\" values(REVIEW_SEQ.NEXTVAL, null, #{categoryno}, #{title}, #{content},#{filename1},#{filename2},#{filename3}, sysdate)")


    @Insert("insert into \"HONG\".\"TEST\" values(REVIEW_SEQ.NEXTVAL, null, #{categoryno}, #{title}, #{content},#{filename1,jdbcType=VARCHAR},#{filename2,jdbcType=VARCHAR},#{filename3,jdbcType=VARCHAR}, sysdate)")
...
//	@Update("update \"HONG\".\"TEST\" set  title = #{title}, content = #{content}, where no = #{no}")
//	void update(Review r);
